...
public class CommonBox {
	...
	public static String[] lbStatus = {
			I18N.getMsg("status.outline"),
			I18N.getMsg("status.draft"),
			I18N.getMsg("status.1st.edit"),
			I18N.getMsg("status.2nd.edit"),
			I18N.getMsg("status.done")
		};
	...
	public static void loadCbStatus(JComboBox cb, Idea idea) {
		String[] lbSt = {
			I18N.getMsg("ideas.status.not_started"),
			I18N.getMsg("ideas.status.started"),
			I18N.getMsg("ideas.status.completed"),
			I18N.getMsg("ideas.status.abandoned")
		};
		cb.removeAllItems();
		for (String x : lbSt) {
			cb.addItem(x);
		}
		if ((idea!=null) && (idea.getStatus()!=null)) cb.setSelectedIndex(idea.getStatus());
	}

	@SuppressWarnings("unchecked")
	public static void fillCbSceneStatus(MainFrame mainFrame, JComboBox cb) {
		String[] lbSt = {
			"status.outline",
			"status.draft",
			"status.1st.edit",
	...
	@SuppressWarnings("unchecked")
	public static void fillCbGenders(MainFrame mainFrame, JComboBox cb) {
		BookModel model = mainFrame.getBookModel();
		Session session = model.beginTransaction();
	...
	@SuppressWarnings("unchecked")
	public static void loadCbCategories(MainFrame mainFrame, JComboBox cb) {
		BookModel model = mainFrame.getBookModel();
		Session session = model.beginTransaction();
	...
	@SuppressWarnings("unchecked")
	public static void loadCbCategories(MainFrame mainFrame, JComboBox cb, Idea idea) {
		BookModel model = mainFrame.getBookModel();
		Session session = model.beginTransaction();
		...
		cb.removeAllItems();
		int ix = -1, i = 0;
		String strIdea = idea!=null?idea.getCategory():"";
		for (String str : ideas) {
			cb.addItem(str);
			if ((idea!=null) && (!strIdea.equals("")) && (strIdea.equals(str))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbCities(MainFrame mainFrame, JComboBox cb, Location location) {
		...
		for (String city : cities) {
			cb.addItem(city);
			if ((location!=null) && (!location.getId().equals(new Long(-1))) && (location.getCity().equals(city))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbSites(MainFrame mainFrame, JComboBox<Location> cb, Location location) {
		...
		for (Location site : sites) {
			cb.addItem(site);
			if ((location!=null) && (!location.getId().equals(new Long(-1))) && (location.getSite().equals(site))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbCountries(MainFrame mainFrame, JComboBox cb, Location location) {
		...
		for (String country : countries) {
			cb.addItem(country);
			if ((location!=null) && (location.getCountry().equals(country))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbCategories(MainFrame mainFrame, JComboBox cb, Item item) {
		...
		for (String category : categories) {
			cb.addItem(category);
			if ((item!=null) && (!item.getCategory().isEmpty()) && (item.getCategory().equals(category))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbCategories(MainFrame mainFrame, JComboBox cb, Tag tag) {
		...
		for (String category : categories) {
			cb.addItem(category);
			if ((tag!=null) && (!tag.getCategory().isEmpty()) && (tag.getCategory().equals(category))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbTags(MainFrame mainFrame, JComboBox cb, Tag tag) {
		...
		for (Tag u : tags) {
			...
			if ((tag!=null) && (!tag.getName().isEmpty()) && (tag.getName().equals(u.getName()))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbItems(MainFrame mainFrame, JComboBox cb, Item item) {
		...
		for (Item u : items) {
			cb.addItem((String)u.getName());
			if ((item!=null) && (!item.getName().isEmpty()) && (item.getName().equals(u.getName()))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbPersons(MainFrame mainFrame, JComboBox cb, Person person) {
		...
		for (Person u : persons) {
			...
			if ((person!=null) && (!person.getFullName().isEmpty()) && (person.getFullName().equals(u.getFullName()))) {
				ix = i;
			}
			i++;
		}
	...
	public static void loadCbScenes(MainFrame mainFrame, JComboBox cb, Scene scene) {
		...
		for (Scene u : scenes) {
			...
			if ((scene!=null) && (!scene.getFullTitle().isEmpty()) && (scene.getFullTitle().equals(u.getFullTitle()))) {
				ix = i;
			}
			i++;
		}
