...
public abstract class AbstractTable extends AbstractPanel implements ActionListener, ListSelectionListener {
	...
	public List<AbstractEntity> getAllEntities() {
		SbApp.trace("AbstractTable.getAllEntities() "+getNewEntity().getClass().getName());
	...
	public void modelPropertyChange(PropertyChangeEvent evt) {
		SbApp.trace(this.getClass().getSimpleName()+".modelPropertyChange("+evt.toString()+")");
	...
	@Override
	public void initUi() {
		SbApp.trace("AbstractTable.initUi()");
		...
		for (SbColumn col : getColumns()) {
			...
			try {
				tcol = table.getColumn(col.toString());
			} catch(IllegalArgumentException ex) {
				System.err.println("Error tcol");
				continue;
			}
			TableCellRenderer renderer = null;
			if ((col.getInputType() == InputType.DATE) &&(! this.getClass().equals(TimeEventTable.class))) {
				renderer = new DateTableCellRenderer();
			} else if ((col.getInputType() == InputType.ICON) &&(! this.getClass().equals(TimeEventTable.class))) {
		...
		for (SbColumn col : getColumns()) {
			...
			try {
				...
				//System.err.println("AbstractTable unknown error for setting colsize");
			}
		}
		...
		btCopy = new JButton("");
		btCopy.setToolTipText(I18N.getMsg("copy"));
		...
		if (hasOrder) {
			...
			btOrderUp.setToolTipText(I18N.getMsg("order.up"));
			...
			btOrderDown.setToolTipText(I18N.getMsg("order.down"));
		...
		if ("Scene".equals(getTableName())) {
			if (BookUtil.isUseXeditor(mainFrame)) {
				btODT = new JButton("");
				btODT.setToolTipText(I18N.getMsg("libreoffice.ok"));
				btODT.setName(ComponentName.BT_ODT.toString());
				btODT.setIcon(I18N.getIcon("icon.small.libreoffice"));
	...
	protected void initTableModel(PropertyChangeEvent evt) {
		SbApp.trace("AbstractTable.initTableModel("+evt.toString()+")");
		...
		try {
			...
			for (AbstractEntity entity : entities) {
				...
				if (entity instanceof Scene) {
					...
					if (!mainFrame.showAllParts && scene.hasChapter()) {
			...
			if (nbTotalObjectif>0) {
			    totalObjectif.setText(I18N.getMsg("manage.size.objective.total")
				    + nbTotalObjectif + " " + I18N.getMsg("characters"));
	...
	public synchronized void actionPerformed(ActionEvent e) {
		...
		SbApp.trace("actCmd="+actCmd+", compName="+compName);
		...
		if (ComponentName.BT_ODT.check(compName) || ActionCommand.LIBREOFFICE.check(actCmd)) {
		...
		} else if (ComponentName.BT_CLOSE.check(compName)) {
			SbApp.trace("view "+getName()+" close");
		}
	}
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (btODT!=null) btODT.setEnabled(b);
	}

	private void sendSetLibreOffice(int row) {
		SbApp.trace("AbstractTable.sendSetLibreOffice("+row+")");
		Scene scene=(Scene) getEntityFromRow(row);
		String odtName=scene.getOdf();
		if (odtName==null || odtName.isEmpty()) {
			odtName=ODTUtils.getDefaultFilePath(mainFrame, scene);
		}
		File file = new File(odtName);
		if (!file.exists()) {
			file=ODTUtils.createNewODT(mainFrame, odtName);
			if (file==null) return;
		...
		try {
			...
			SbApp.error("AbstractTable.sendSetLibreOffice(" + mainFrame + scene.getFullTitle() + ")", e);
		}
	}
	...
	private void createOdtFile(File file) {
		...
		try {
			...
			if (BookUtil.isUsePersonnalTemplate(mainFrame)) {
				Internal internal = BookUtil.get(mainFrame, SbConstants.BookKey.USE_PERSONALTEMPLATE, "");
